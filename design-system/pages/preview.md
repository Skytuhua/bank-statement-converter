## OVERRIDE — Preview step (MASTER governs unless noted)
- Dense **data table**: IBM Plex Mono 0.8125rem, tabular-nums, amounts right-aligned and colored by inflow(green)/outflow(red) semantics only.
- Sticky header; zebra rows via subtle muted bg; horizontal scroll on narrow screens.
- Summary bar above table: row count, total inflow, total outflow, net; plus a warnings badge (unparsed dates/amounts) that filters to problem rows.
- Empty state when no rows parsed; never a frozen/blank table.
